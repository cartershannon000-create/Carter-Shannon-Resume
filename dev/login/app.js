...
function renderOverview(){
  ...
  <section class="explain-section"><div class="section-label"><strong>How to read the overview</strong><span>Definitions, calculation boundaries, and decision relevance</span></div><div class="explain-grid">
    ${explainerCard('Active work','Durable continuity tasks whose current status is active.',`${number(tasks.length)} active tasks`,`${number(allTasks.length)} total tracked tasks`,'Shows the amount of work that currently needs ownership and a next action; it does not measure productivity.')}
    ${explainerCard('Pending review','Audit recommendations still awaiting a human disposition.',`${number(o.pending_review)} proposed recommendations`,`${number(recommendations.length)} total recommendations`,'Makes the human decision backlog visible so useful recommendations do not remain inert.')}
    ...
    ${explainerCard('Captured tokens','Input, output, cache-read, and cache-write tokens recorded across Claude and Codex.',`${compact(o.captured_tokens)} recorded tokens`,`${number(tokenized)} tokenized events ÷ ${number(allEvents)} total events for coverage`,'Explains the scale and composition of model processing. Token volume alone is not a measure of value or quality.')}
  </div></section>
