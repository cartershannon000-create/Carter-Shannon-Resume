...
function bindOfficeForms(scope,clientId=null){
  ...
  $$('[data-office-form="calendar"]',scope).forEach(form=>{form.onsubmit=submitCalendar;const kind=form.elements.kind,done=$('.calendar-done',form),sync=()=>{if(done)done.hidden=kind.value!=='reminder'};kind.onchange=sync;sync()});
...
async function reloadOffice(reopenClientId=null){const{data:office,error}=await sb.rpc('api_office_state');if(error)throw error;officeState=office||{clients:[],calendar:[]};renderClients();renderFinances();renderCalendar();bindNavigation();if(reopenClientId)openClientDrill(reopenClientId)}
