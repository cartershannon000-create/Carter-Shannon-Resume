...
function renderApprovals(){
  ...
  <div class="card release-card"><div class="card-head"><div><h3>Release approvals</h3><p>Review the run log and delivery evidence, then confirm the finished work.</p></div><span class="chip">${releases.length} waiting</span></div>${releases.length?`<div class="approval-list">${releases.map(work=>`<div class="approval-row"><i class="priority-dot high"></i><div><strong>${esc(work.title||work.work_id)}</strong><p>Finished ${date(work.updated_at)} · approving marks the ledger completed</p></div><div class="action-buttons"><button class="link-button" data-run="${esc(work.work_id)}" data-run-title="${esc(work.title)}">View evidence</button><button class="action-button approve" data-release-work="${esc(work.work_id)}" data-release-title="${esc(work.title)}">Approve release</button></div></div>`).join('')}</div>`:'<div class="empty-state"><strong>No release approvals waiting</strong><p>Finished work appears here after execution and verification.</p></div>'}<div class="notice release-notice">Approving release records your acceptance and marks the work completed. It does not rerun an agent, merge a pull request, or deploy code; confirm delivery first.</div></div>
  ...
  <div class="grid-2"><article class="card"><div class="card-head"><div><h3>Execution approvals</h3><p>Select the starting model, then approve. The choice is bound to the queued attempt.</p></div></div>${gates.length?`<div class="approval-list">${gates.map(a=>`<div class="approval-row"><i class="priority-dot ${a.gate_type==='recovery'?'critical':'high'}"></i><div><strong>${esc(a.title||a.work_id)}</strong><p>${esc(a.gate_type)} gate · hash ${esc((a.payload_hash||'').slice(0,12))}… · ${date(a.requested_at)}</p></div><div class="approval-controls"><label class="model-choice">Start model<select data-start-provider><option value="claude" selected>Claude</option><option value="codex">Codex</option></select></label><div class="action-buttons"><button class="action-button approve" data-approval="${esc(a.approval_id)}" data-decision="true">${a.gate_type==='recovery'?'Approve retry':'Approve & run'}</button><button class="action-button reject" data-approval="${esc(a.approval_id)}" data-decision="false">Reject</button></div></div></div>`).join('')}</div>`:'<div class="empty-state"><strong>No execution approvals waiting</strong><p>Accept a recommendation to create one.</p></div>'}</article>
