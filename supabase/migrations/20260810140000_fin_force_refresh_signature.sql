-- fin.sync_plaid gains p_force_refresh, which asks Plaid to re-poll the institution
-- before reading. /transactions/sync only returns what Plaid has already collected on
-- its own schedule, so without this an explicit refresh could not surface a charge the
-- card issuer was already showing -- a Lyft charge posted on Amex while Plaid still
-- reported it pending, and no amount of syncing moved it.
--
-- Rate limited, so only a deliberate refresh pays for it; a passive tab view still does
-- the cheap read. A failure to re-poll does not stop the sync: stale data beats none.
...
drop function if exists fin.sync_plaid(boolean);
